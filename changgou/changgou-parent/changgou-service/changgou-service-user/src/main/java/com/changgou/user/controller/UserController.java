...
public class UserController {
    ...
    /**加密存储bcrypt方式密文相同密码都不同
     *用户登录
     */
    @GetMapping(value = "/login")
    public Result login(String username, String password, HttpServletResponse response){
        //查询用户信息   根据主键username查询
        User user = userService.findById(username);
        //对比密码,加密
//        if (password.equals(user.getPassword())){
        //调用加密工具BCrypt.checkpw对比密码
        if (BCrypt.checkpw(password,user.getPassword())){
            ...
            //创建令牌   HttpServletResponse response
            ...
            return new Result(true,StatusCode.OK,"登录成功",token); //令牌作为参数

        }
        //密码不匹配，登录失败，账号或密码错误
        return new Result(false,StatusCode.LOGINERROR,"账号或用户名错误");
